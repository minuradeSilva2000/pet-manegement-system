...
function Grooming() {
  ...
  // Update amount when grooming type changes
  useEffect(() => {
    if (groomingType && servicePrices[groomingType]) {
      setAmount(servicePrices[groomingType]);
    } else {
      setAmount(0);
    }
  }, [groomingType]);
    ...
    <div className="p-4" style={{ backgroundColor: 'var(--main-color)', color: 'var(--grey)' }}>
      ...
      <div className="bg-[var(--background-light)] p-6 rounded-lg shadow-md max-w-md mx-auto mt-8 mb-8">
        ...
        <form onSubmit={setData} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={!groomingType || !petId || !date || !time}
            className={`w-full py-2 rounded-md font-semibold transition
              ${!groomingType || !petId || !date || !time 
